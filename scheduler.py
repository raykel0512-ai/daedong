# scheduler.py — 시험 시감 자동 배정 알고리즘 v3.1
...
def run_assignment(
    ...
    for (d, p) in slots:
        ...
        for (g, c) in active_slots:
            chief_name = "(미배정)"
            # 정렬 순서: 정감독 누적 횟수 -> 롤링 인덱스(가장 중요) -> 우선순위
            sorted_chiefs = sorted(chief_pool, key=lambda t: (running_chief[t.name], (orig_idx_map[t.name] - last_idx) % total_t, t.priority))
            for t in sorted_chiefs:
                ...
                if can_assign(t, d, p, g, c):
                    ...
                    # ★ 배정이 일어날 때마다 인덱스를 갱신하여 다음 반은 다음 사람부터 검토하도록 함
        ...
        for (g, c) in active_slots:
            asst_name = "(미배정)"
            # 정렬 순서: 역할(학부모0, 교사1) -> 부감독 누적 횟수 -> 롤링 인덱스 -> 우선순위
            sorted_assts = sorted(asst_pool, key=lambda t: (0 if t.role == "학부모" else 1, running_asst[t.name], (orig_idx_map[t.name] - last_idx) % total_t, t.priority))
