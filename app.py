...
# 실제 배정 — "완화" 우선순위(라운드로빈 + 저우선 리필)
classroom_assignments = dict()  # (d,p) -> dict[(g,c)] = (chief, assistant)
class_cursor = 0  # 라운드로빈 커서
...
for (d, p) in slots:
    ...
    for g in active_grades:
        for c in range(1, classes_per_grade + 1):
            pair = []  # [chief, assistant]

            # 1차(정규): remaining_quota>0인 교사를 라운드로빈으로 선발
            checked = 0
            while len(pair) < 2 and checked < len(teacher_order) * 8:
                t = teacher_order[class_cursor % len(teacher_order)]
                class_cursor += 1
                checked += 1

                need_role = 0 if len(pair) == 0 else 1  # 0=정, 1=부
                q = chief_quota if need_role == 0 else assistant_quota
                if q.get(t, 0) <= 0:
                    continue
            ...
            # 2차(리필): quota 무시, **우선 낮은 사람부터(reversed)**로 빈칸 메움
            if len(pair) < 2:
