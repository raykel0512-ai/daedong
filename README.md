...
| `extra_classes` | ❌ | 기피반 (해당 반 자동배정 제외) | `1-4~6` |
...
### 기피반 (extra_classes)
해당 교사를 이 학급의 정/부감독으로 **자동배정하지 않습니다** (회피).
열 이름은 과거 호환을 위해 `extra_classes`로 유지하나, 동작은 '추가 배정'이 아닌 '제외'입니다.
- `1-4~6` → 1학년 4, 5, 6반 회피
- `2-1,2-2` → 2학년 1, 2반 회피
...
- ✅ 날별 교사 명단 분리 업로드
- ✅ `role` 열: 정부 / 부만 구분
- ✅ `extra_classes` 열: 특정 반 회피(자동배정 제외)
